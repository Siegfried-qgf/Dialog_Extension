...
def cat_datacc(data1,data2):
    data1=load_json(data1)
    data2=load_json(data2)
    print("ConvQA :" + str(len(data1)))
    print("Squad :" + str(len(data2)))
    data1.update(data2)
    print(len(data1))
    save_json(data1,"./../CQA/QA_CO_SQ/train_QA_CO_SQ.json")

cat_datacc(path_qa+'train_QA.json',path_squad+"train_QA_S.json")
'''