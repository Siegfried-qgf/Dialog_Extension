...
from torch.utils.data import IterableDataset, Sampler, SequentialSampler, RandomSampler,  Dataset
...
class DataLoader(Generic[T_co]):
    ...
    def __init__(self, dataset: Dataset[T_co], batch_size: Optional[int] = 1,
        ...
        if batch_size is not None and batch_sampler is None:
            ...
            batch_sampler = MyBatchSampler(dataset , sampler, batch_size, drop_last)
...
class MyBatchSampler(Sampler[List[int]]):
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        if len(batch2) > 0 and not self.drop_last:
            yield batch2
        if len(batch3) > 0 and not self.drop_last:
            yield batch3
        if len(batch4) > 0 and not self.drop_last:
            yield batch4
        if len(batch5) > 0 and not self.drop_last:
            yield batch5
