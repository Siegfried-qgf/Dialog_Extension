...
class dataset(object):
    def __init__(self,bool,filename,opt):
        if bool:
            '''
    ...
    def padding_context(self,contexts,pad=0,transformer=True):
        ...
        else:  #这里contexts是两层list  [['Hi', 'I', 'am', 'looking', 'for', 'a', 'movie', 'like', '@111776']]
            contexts_com=[]
            for sen in contexts[-self.max_count:-1]:
                contexts_com.extend(sen)
                contexts_com.append('_split_')
            contexts_com.extend(contexts[-1])
            vec,v_l,concept_mask,dbpedia_mask=self.padding_w2v(contexts_com,self.max_c_length,transformer)#concept_mask对应Key2index_3rd   dbpedia_mask对应电影的id  vec对应word2index_redial
