...
from KGSF.model import CrossModel
...
class TrainLoop_fusion_rec():
    def __init__(self, opt, is_finetune):
        ...
        self.train_dataset=dataset(True,'./data/CRS/ReDial/train_data.jsonl',opt)
        #self.train_dataset = dataset(True, './data/train_data.jsonl', opt)
        #词表
        # 4/21
    ...
    def metrics_cal_rec(self,rec_loss,scores,labels):
        ...
        infer_movie = pred_idx.numpy().tolist()
        for b in range(batch_size):
            if labels[b].item()==0:
                self.movie_rec_list.append([])
                continue
            else:
                top_10 = []
                for i in range(10):
                    if int(infer_movie[b][i])!=labels[b].item():
                        top_10.append(self.movie_ids[int(infer_movie[b][i])])
                if len(top_10)==10:
                    top_10.pop()
                    top_10.append(int(labels[b].item()))
                else:
                    top_10.append(int(labels[b].item()))
                for i in range(len(top_10)):
                    top_10[i] = self.id2movie(top_10[i])
                random.shuffle(top_10)
                self.movie_rec_list.append(top_10)
    ...
    def val(self,is_test=False):
        ...
        if is_test:
            val_dataset = dataset(True,'data/test_data.jsonl', self.opt)
        else:
            val_dataset = dataset(True,'data/valid_data.jsonl', self.opt)
    ...
    def top10(self,context,response,mask_response,concept_mask,dbpedia_mask, entity, movie, concept_vec, db_vec, entity_vector, rec):
        self.model.eval()
        with torch.no_grad():
            recs=[]
            seed_sets = []
            ...
            for b in range(batch_size):
                ...
                seed_sets.append(seed_set)
            scores, preds, rec_scores, rec_loss, _, mask_loss, info_db_loss, info_con_loss = self.model(
                    context.cuda(), response.cuda(),
                    mask_response.cuda(), concept_mask, dbpedia_mask, seed_sets, movie, concept_vec, db_vec,
                    entity_vector.cuda(), rec, test=True, maxlen=20, bsz=batch_size)
            outputs = rec_scores.cpu()
            outputs = outputs[:, torch.LongTensor(self.movie_ids)]
            _, pred_idx = torch.topk(outputs, k=100, dim=1)
            infer_movie = pred_idx.numpy().tolist()
            top10=[i for i in infer_movie[0]][:10]
            for n in range(10):
                top10[n]=self.id2movie(self.movie_ids[int(top10[n])])
        ...
        for context, c_lengths, response, r_length, mask_response, mask_r_length, entity, entity_vector, \
            ...
            with torch.no_grad():
                ...
                for b in range(batch_size):
                    seed_set = entity[b].nonzero().view(-1).tolist()
                    seed_sets.append(seed_set)
                _, _, _, _, gen_loss, mask_loss, info_db_loss, info_con_loss = self.model(context.cuda(), response.cuda(),
                mask_response.cuda(), concept_mask, dbpedia_mask, seed_sets, movie, concept_vec, db_vec, entity_vector.cuda(), rec, test=False)
                scores, preds, rec_scores, rec_loss, _, mask_loss, info_db_loss, info_con_loss = self.model(context.cuda(), response.cuda(),
                mask_response.cuda(), concept_mask, dbpedia_mask, seed_sets, movie, concept_vec, db_vec, entity_vector.cuda(), rec, test=True, maxlen=20, bsz=batch_size)
...
if __name__ == '__main__':
    ...
    loop.model.load_model()
    loop.val(True)
    save_json(loop.movie_rec_list,"movie_rec_list_test.json")
